...
function MedicalRecords({ patient, onEdit, onDelete, onPatientUpdate }) {
  ...
  useEffect(() => {
    if (patient?.id) {
      ...
      axios.get(`${import.meta.env.VITE_API_BASE_URL}/get_all_medical_records.php?patient_id=${patient.id}`)
        .then((res) => {
  ...
  const handleViewRecord = async (recordId) => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/get_medical_record_by_id.php?record_id=${recordId}&patient_id=${patient.id}`);
  ...
  const toggleEdit = async () => {
    if (isEditing) {
        ...
        const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/update_patient_comprehensive.php`, formData);
