...
export const preloadAllData = async () => {
  const preloadTasks = [
    preloadData('patients', () => 
      fetch(`${import.meta.env.VITE_API_BASE_URL}/get_patients.php`).then(r => r.json())
    ),
    preloadData('dashboard', () => 
      fetch(`${import.meta.env.VITE_API_BASE_URL}/get_dashboard_data.php`).then(r => r.json())
    ),
    preloadData('diseases', () => 
      fetch(`${import.meta.env.VITE_API_BASE_URL}/get_diseases.php`).then(r => r.json())
    ),
    preloadData('reports', () => 
      fetch(`${import.meta.env.VITE_API_BASE_URL}/get_reports_data.php`).then(r => r.json())
    )
  ];
