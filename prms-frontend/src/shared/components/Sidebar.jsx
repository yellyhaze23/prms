...
import { FaHospital, FaChevronRight } from 'react-icons/fa';
...
export default function Sidebar({ nav = [], brandTitle = 'RHU PRS', brandSubtitle = 'Patient Record System' }) {
  return (
    <div className="fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 shadow-2xl border-r border-slate-700/50 z-50 hidden lg:block">
      ...
      {/* Footer */}
      <div className="p-4 border-t border-slate-700/50 text-xs text-slate-500">&copy; {new Date().getFullYear()}</div>
    </div>
  );
}
